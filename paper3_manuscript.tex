...
\textcolor{red}{In this paper, a dual TSM (DTSM) controller is proposed for a class of second-order nonlinear systems with disturbances and uncertainties. A novel DTSM surface is exploited to achieve the finite time convergence, and the reaching and sliding times can both be calculated directly. For coping with the underacutated issue, cooperating with the hierarchical sliding surface design methodology, we develop a hierarchical dual TSM (HDTSM) control to ensure the rigorous stability analyses. A modified DTSM (MDTSM) control scheme is presented to regulate the motion of the $n$-link robotic manipulators, and the case subjected to input limitations is also taken into consideration. The remained paper is organized as follows: Section~\ref{sec:2} introduces a DTSM scheme for a class of second-order system, and the finite time convergences of the reaching and sliding phase are verified. Furthermore, we design extra performance parameters to obtain the MDTSM scheme, and the newly presented method has a better performance in theoretical analyses; for stabilizing the underactuated system, the HDTSM design is also developed in this section. In Section~\ref{sec:3}, the MDTSM is applied to the motion control system of the $n$-link robotic  manipulator, and the stability analysis is also carried out. Section~\ref{sec:4} exhibits the simulation results, and finally the conclusion is assigned in Section~\ref{sec:5}.}
...
\end{split}
\end{align}
where $\bm x = [x_1,x_2,x_3,x_4]^T$ is the system vector state; $f(\bm x,t)$ and $b(\bm x,t)\neq 0$ are smooth nonlinear functions with respect to $\bm x$; $u_i(t)$, $i=1,2$ stands for the scalar input; $g_i(\bm x,t)$, $i=1,2$  is defined as a bounded nonlinear function which satisfies $\Vert g_i(\bm x,t)\Vert\le l_i$, $l_i>0, i=1,2$, meaning the uncertainty and disturbance of the system. For the system mentioned above, we can establish a desired dual terminal sliding surface as:
\begin{align}
\bm s = [s_1,s_2]^T,
\end{align}
where
...
s_2 &= x_4+\vert x_3\vert^{\alpha} sgn(x_3)+x_3^3(t)+x_1,\label{eq:dual sliding surface}
\end{split}
\end{align}
where $sgn(x)$ is the sign function, and $0<\alpha<1$ is a positive constant. For the sake of simplicity, the symbol $t$ is usually omitted in this paper, which does not cause any confusion, and it will appear once the derivation concerns for the factor of the time strictly, i.e., the desired sliding surface $\bm s$ is often abbreviated to $\bm s$.\par
...
\end{align}
where the definitions of  $t_s$, $t_0$, $\alpha$ are similar to Lemma \ref{lemma:1}. Introducing the amplitude scalar $d$ into the Eqs.~(\ref{eq:normal sliding surface}) obtains  more normal forms:
\begin{align}
...
\begin{mythm}\label{theorem:1}Dual Terminal SMC Design.
The second-order system (\ref{eq:second-order system}) will reach the desired surface $\bm s = 0$ shown in~Eq. (\ref{eq:dual sliding surface}) in finite time. The states of the system will converge to zero along the surface $\bm s=0$ in finite time, if the inputs are designed as follows:
\begin{align}
\begin{split}
...
\end{align*}
Hence, the system (\ref{eq:second-order system}) asymptotically converges to the desired surface (\ref{eq:dual sliding surface}), and incidentally the convergence time is calculated according to the finite time convergence theory.\par
Once the system (\ref{eq:second-order system}) reaches the desired surface~(\ref{eq:dual sliding surface}), the whole system is handled by the reduced system:
\begin{align*}
\dot x_1&=-\vert x_1\vert^\alpha sgn(x_1)-x_1^3+x_3\\
\dot x_3&=-\vert x_3\vert^\alpha sgn(x_3)-x_3^3-x_1.
\end{align*}\par
The total convergence time can be calculated by dividing the convergence into two phases with respect to the relation between the system and the surface, namely, the reaching phase and the sliding phase, and specifically, according to Lemma \ref{lemma:2}, the total finite convergence time $t_f$ is calculated from:
\begin{align}
...
\end{align}
where $\hat V(t) = \frac{x_1^2+x_3^2}{2}$ is the Lyapunov function describing the convergence motion of the reduced system. The reaching and sliding time are defined as $t_r$ and $t_s$, respectively. This completes the proof.
\end{proof}
On the basis of Theorem \ref{theorem:1}, we proceed to develop the parameters in the surface to improve the performance, which introduces a modified method for stabilizing the system~(\ref{eq:second-order system}), and furthermore this method makes the design of the surface more flexible. Consider the system~(\ref{eq:second-order system}) with the surface $\bm s = [s_1,s_2]^T$, the details of which are described by following expressions:
\begin{align}
\begin{split}
