...
\begin{align}
\bm {M(q)\ddot q}+\bm {C(q,\dot q)}+\bm {g(q)}=\bm {\tau}(t)+\bm{d}(t),
\end{align}
where $\bm q(t)$ is the $2\times 1$ vector of joint angular position; $\bm {M(q)}$ is the $2\times 2$ inertia matrix; $\bm{C(q,\dot q)}$ denotes the Coriolis and centrifugal forces by using the $n\times1$ vector; $\bm{g(q)}$ stands for the $n\times 1$ gravitational torque; $\bm{\tau}(t)$ is referred to as the joint torques which are the inputs practically; $\bm d(t)$ expresses the $n\times 1$ bounded input disturbances vector.
