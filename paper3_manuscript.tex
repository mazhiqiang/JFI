...
(...background of sliding mode control...)\par
(...sliding mode control develop...)\par
Recent years, the sliding mode control has become a popular solution to the issue about various control and signal process missions \cite{zhao2015nonlinear,zhang2015attitude}. Meza-S{\'a}nchez et al. applied the second sliding mode controller to a 3-DOF helicopter prototype, and verified the effectiveness via the experimental test bed \cite{meza2015output}. Zheng et al. implemented sliding mode control to eliminate the effect of the input quantization generated between the coder and the decoder \cite{zheng2016sliding}. Guzman et al. investigated the application of a sliding mode control in a power factor rectifier with constant switching frequency \cite{guzman2016sliding}. Said et al. designed a robust adaptive sliding mode method for a boost converter to guarantee the asymptotically stability \cite{oucheriah2013pwm}.  high order sliding mode control \cite{castillo2015higher,palosz2015laser}, super-twisting sliding mode\cite{zhao2015finite,liu2015second}
(...the terminal sliding mode develop...)\par
Normal linear hyperplane-based sliding mode control takes two steps to solve a problem about the stability, namely, conducting the system states onto the prescript sliding manifold and driving these states to the origin along the manifold. The first step is said to be the approaching phase which usually requires the states asymptotically or exponentially converge to the desired manifold, and the second step is the sliding phase which is related to the designed manifold closely. For achieving a fast convergence, the terminal sliding mode (TSM) control is developed based on the finite time control \cite{haimo1986finite,bhat1997finite,mu2016switching}, and comparing with the sliding mode control design mentioned above, the most significant contributions of the terminal sliding mode is introducing the finite time convergence to the sliding phase. Owing to the fast convergence, the terminal sliding mode control has been utilized to achieve the high precise control, i.e., in \cite{li2015robust}, an adaptive TSM scheme controlled a $2$-link robotic manipulator to track the variable signal. \par
In this paper, a dual sliding mode (DSM) controller is proposed for a class of two-order nonlinear systems with disturbances and uncertainties. A novel DSM manifold is exploited to fulfill the finite time convergence, and the approaching and sliding time are both able to be calculated directly. A modified DSM (MDSM) control scheme is presented to stabilize the motion of the $n$-link robotic manipulators. The remained paper is organized as follows: Section~\ref{sec:2} introduces a DSM scheme for a class of two-order system, and the finite time convergence of the approaching and sliding is verified. In addition, we add performance parameters for improving the DSM scheme to obtain the MDSM scheme, and the newly presented method has a better performance in theoretical analysis. In Section~\ref{sec:3}, the MDSM is applied to the motion regulation of the $n$-link robotic  manipulator, and the stability analysis is also carried out. Section~\ref{sec:4} exhibited the simulation results, and finally the conclusion is assigned in Section~\ref{sec:5}.
\section{Dual sliding mode control}\label{sec:2}
...
\end{mylem}
For stabilization of the two-order system shown as~Eq. (\ref{eq:two order system}), dual sliding mode control inputs can be given according the following theorem.
...
Consider a two-order system (\ref{eq:two order system}), and for eliminating the singularity in inputs of the MDSM scheme, select a new inputs to complete the nonsingular MDSM scheme. Introducing the saturation terms into the original inputs yields:
\begin{align}
...
\end{proof}
\section{Dual sliding mode control for rigid manipulators}\label{sec:3}
In this section, a dual sliding mode control is proposed for the rigid $2$-link robotic manipulator
\begin{align}
...
\end{align*}
On the basis of the above results, one can design the dual sliding mode scheme for $2$-link robotic manipulator by utilizing the following theorem.
...
\end{proof}
\begin{myrem}
At this point, the dual sliding mode control scheme for $2$-link robotic manipulator has been demonstrated, and this technique is able to be extended to $n$-link manipulator. Consider a $n$-link robotic manipulator in where $n$ is an even number, and the desired sliding manifold can be designed as
\begin{align}
...
\end{myrem}
\begin{myrem}
The singularity of the DSM scheme for the robotic manipulator can be eliminated by using the technique introduced in Theorem \ref{theorem:3}.
\end{myrem}
\begin{myrem}
...
\end{split}
\end{align}
and one can produce a MDSM control scheme conveniently. It is obvious that the disturbance in the system~(\ref{eq:simulation two order system}) behaves bounded, which is said to be the following uncertainty terms:
\begin{align*}
...
At this point, some effectiveness of the MDSM control scheme proposed above has been verified, and in the next section, we will apply it to stabilize the behavior of a $2$-link robotic manipulator directly.
\begin{figure}
\centering
...
The performance of the controllers mentioned above is illustrated in Figure~\ref{Figure:4} and Figure~\ref{Figure:5}. Comparatively speaking, $q_1$ reaches the desired state at about $3s$ with the MDSM scheme, the reaching time of which is approximating $0.8s$ less than of the TSM scheme, and $q_2$ performs similar behaviors with a reasonable variable range of the angular rates. One can concentrate on the mission time of the entire motion, and inductively the time is decided by the manipulator finishing the final motion, i.e., for the manipulator controlled by utilizing the MDSM scheme, one can obtain the mission time $3s$ approximately, comparing with $3.25s$ for the TSM control. More notably, the control parameters of the TSM scheme have been optimally selected, whereas if using the parameters for the MDSM scheme, one can only get a fairy poor performance in the regulation, which leads to meaningless comparison. Therefore these results show that the MDSM control scheme can achieve the regulation of the manipulator effectively.
\begin{figure}
\centering
